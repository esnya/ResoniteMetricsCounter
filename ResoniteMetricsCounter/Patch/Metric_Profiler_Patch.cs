...
internal static class Metric_Profiler_Patch
{
    ...
    private static CodeMatcher InjectProfiler(this CodeMatcher matcher, CodeMatch match)
    {
        ...
        return matcher
            ...
            .Advance(1)
            .InsertAndAdvance(CodeInstruction.Call(() => Record(default!)));
    }

    private static IEnumerable<CodeInstruction> InjectProfiler(
    ...
    private static CodeMatcher InjectProfiler(
    ...
    {
        internal static readonly MethodBase updatesTarget = typeof(UpdateManager).Method(
            nameof(UpdateManager.RunUpdates)
        );
        internal static readonly MethodBase changesTarget = typeof(UpdateManager).Method(
            "ProcessChange"
        );
        internal static readonly MethodBase connectorsTarget = typeof(UpdateManager).Method(
            "ProcessConnectorUpdate"
        );
        ...
        )
        {
            if (__originalMethod == updatesTarget)
            {
                ...
                return instructions.InjectProfiler(
                    CodeMatch.Calls(() => default(IUpdatable)!.InternalRunUpdate())
                );
            }
            else if (__originalMethod == changesTarget)
            {
                ...
                return instructions.InjectProfiler(
                    1,
                    CodeMatch.Calls(() => default(IUpdatable)!.InternalRunApplyChanges(default))
                );
            }
            else if (__originalMethod == connectorsTarget)
            {
                ...
                return instructions.InjectProfiler(
                    1,
                    CodeMatch.Calls(() => default(IImplementable)!.InternalUpdateConnector())
                );
            }
    ...
    {
        internal static readonly MethodBase rebuildChangeTrackingTarget =
            typeof(ProtoFluxController).Method(nameof(ProtoFluxController.RebuildChangeTracking));
        internal static readonly MethodBase rebuildTarget = typeof(ProtoFluxController).Method(
            nameof(ProtoFluxController.Rebuild)
        );
        internal static readonly MethodBase eventsTarget = typeof(ProtoFluxController).Method(
            nameof(ProtoFluxController.RunNodeEvents)
        );
        internal static readonly MethodBase updatesTarget = typeof(ProtoFluxController).Method(
            nameof(ProtoFluxController.RunNodeUpdates)
        );
        internal static readonly MethodBase continuousChangesTarget =
            typeof(ProtoFluxController).Method(nameof(ProtoFluxController.RunContinuousChanges));
        internal static readonly MethodBase discreteChangesTarget =
            typeof(ProtoFluxController).Method(nameof(ProtoFluxController.RunDiscreteChanges));
        ...
        internal static IEnumerable<MethodBase> TargetMethods()
        {
            ...
            if (
                ResoniteMetricsCounterMod.GetStageConfigValue(
        ...
        )
        {
            if (__originalMethod == rebuildChangeTrackingTarget)
            {
                ...
                return instructions.InjectProfiler(
                    CodeMatch.Calls(() => default(ProtoFluxNodeGroup)!.RebuildChangeTracking())
                );
            }
            else if (__originalMethod == rebuildTarget)
            {
                return instructions.InjectProfiler(
                    CodeMatch.Calls(() => default(ProtoFluxNodeGroup)!.Rebuild())
                );
            }
            else if (__originalMethod == eventsTarget)
            {
                ...
                return instructions.InjectProfiler(
                    CodeMatch.Calls(() => default(ProtoFluxNodeGroup)!.RunNodeEvents())
                );
            }
            else if (__originalMethod == updatesTarget)
            {
                ...
                return instructions.InjectProfiler(
                    CodeMatch.Calls(() => default(ProtoFluxNodeGroup)!.RunNodeUpdates())
                );
            }
            else if (__originalMethod == continuousChangesTarget)
            {
                ...
                return instructions.InjectProfiler(
                    CodeMatch.Calls(() => default(ProtoFluxNodeGroup)!.RunNodeChanges())
                );
            }
            else
            {
    ...
    [
        HarmonyPatchCategory("ParticleSystems"),
        HarmonyPatch(typeof(ParticleSystemManager), nameof(ParticleSystemManager.Update))
    ]
    internal static class ParticleSystemManager_Patch
    {
        internal static IEnumerable<CodeInstruction> Transpiler(
            ...
            instructions.InjectProfiler(
                CodeMatch.Calls(typeof(ParticleSystem).Method("ScheduleUpdate"))
            );
    }
    ...
    {
        internal static readonly MethodBase updateTarget = typeof(DynamicBoneChainManager).Method(
            nameof(DynamicBoneChainManager.Update)
        );
        internal static readonly MethodBase simulateChainTarget =
            typeof(DynamicBoneChainManager).Method("SimulateChain");

        internal static IEnumerable<MethodBase> TargetMethods()
        {
            if (
                ResoniteMetricsCounterMod.GetStageConfigValue(MetricStage.DynamicBoneChainPrepare)
                || ResoniteMetricsCounterMod.GetStageConfigValue(MetricStage.DynamicBoneChainFinish)
            )
            {
        ...
        )
        {
            if (__originalMethod == updateTarget)
            {
                ...
                {
                    instructions = instructions.InjectProfiler(
                        MetricStage.DynamicBoneChainPrepare,
                        CodeMatch.Calls(typeof(DynamicBoneChain).Method("Prepare"))
                    );
                }
                ...
                )
                {
                    instructions = instructions.InjectProfiler(
                        MetricStage.DynamicBoneChainFinish,
                        CodeMatch.Calls(typeof(DynamicBoneChain).Method("FinishSimulation"))
                    );
                }
            ...
            else if (__originalMethod == simulateChainTarget)
            {
                ...
                return instructions.InjectProfiler(
                    MetricStage.DynamicBoneChainSimulation,
                    CodeMatch.Calls(typeof(DynamicBoneChain).Method("RunSimulation"))
                );
            }
    ...
    {
        internal static MethodBase TargetMethod() =>
            typeof(DynamicBoneChainManager).Inner("CollisionHandler").Method("Handle");
        ...
        internal static IEnumerable<CodeInstruction> Transpiler(
            ...
            instructions.InjectProfiler(
                ...
                CodeMatch.Calls(typeof(DynamicBoneChain).Method("ScheduleCollision"))
            );
    }
}
